...
import { buildGptPrompt } from "./criteria.js";
...
const NAV=[
  ...
  {g:"활동 분析",items:[{id:"s3",ico:"✦",lbl:"강점·보완점"},{id:"s4",ico:"📅",lbl:"활동 타임라인"},{id:"s5",ico:"🔑",lbl:"키워드 분析"}]},
  {g:"역량·패턴",items:[{id:"s6",ico:"⚡",lbl:"역량 채점"},{id:"s7",ico:"🏆",lbl:"합격 패턴"}]},
...
export default function App() {
  ...
  const run=useCallback(async()=>{
    ...
    setBusy(true); setErr(""); setProg(8); setStep("생기부 텍스트 파싱 중...");
    try{
      // ⚡ 변경 1: 로컬 파싱 시도 (실패해도 진행)
      let parsed = await parseStudentRecord(pdfB64);
      // ⚡ 변경 2: 텍스트 추출 실패해도 Claude한테 PDF 직접 보내기
      // (이미지 기반/보안 PDF도 Claude는 직접 읽을 수 있음)
      if(!parsed || !parsed.rawText || parsed.rawText.length < 300){
        setStep("로컬 파싱 약함 — Claude가 PDF 직접 분석합니다...");
        parsed = parsed || {};
      ...
      setProg(28); setStep("AI 분석 준비 중...");

      const prompt=buildGptPrompt(parsed,inName||parsed.studentInfo?.name,inMajor,inCurr,ranks);
      setProg(35); setStep("AI가 학생부 원문 분析 중... (30~60초 소요)");

      const resp=await fetch("/api/analyze",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({pdfB64,prompt,pdfText:parsed.rawText,parsed:{grades:parsed.grades,achievementSubjects:parsed.achievementSubjects,gradeAvg:parsed.gradeAvg,schoolName:parsed.studentInfo?.school}})
      });
      setProg(72); setStep("분析 결과 처리 중...");
      ...
      let json={};
      try{json=JSON.parse(clean);}catch{const m=clean.match(/\{[\s\S]*\}/);if(m)json=JSON.parse(m[0]);else throw new Error("JSON 파싱 실패");}
      if(json.studentName&&!inName) setInName(json.studentName);
      // ⚡ 변경 3: 로컬파서 덮어쓰기 제거 ★ 핵심 ★
      // 예전엔 Claude가 PDF 보고 정확히 뽑은 grades를
      // "로컬파서가 더 많이 뽑았으면 덮어쓰기"로 망가뜨림. 그거 제거.
      // 이제 Claude의 PDF 직접 분석 결과를 그대로 신뢰함.
      //
      // ⚡ 변경 4: Claude가 비워서 보냈을 때만 로컬파서를 폴백으로 사용
      // (이미지 PDF에서 Claude도 못 읽었을 극단적 경우)
      ...
      setG(json); setProg(100); setStep("✅ 분析 완료!");
      setPhase("result");
      setTimeout(()=>{resRef.current?.scrollIntoView({behavior:"smooth"});setProg(0);setStep("");},600);
    }catch(e){setErr(String(e.message||e).slice(0,150));setProg(0);setStep("");}
    setBusy(false);
  ...
  useEffect(()=>{
    ...
    const avgOf=(sk,grp)=>{const[g,s]=sk.split("-");const f=gs.filter(r=>String(r.gN)===g&&String(r.sN)===s&&(!grp||r.group===grp)&&parseInt(r.level||0)>0);return f.length?+(f.reduce((a,b)=>a+parseInt(b.level),0)/f.length).toFixed(2):null;};
    const grps=Object.keys(PAL).filter(g=>gs.some(r=>r.group===g));
    ...
    if(rTrend.current){
      ...
      ds.unshift({label:"전체평균",data:sks.map(sk=>avgOf(sk,null)),borderColor:"#1a1d2e",backgroundColor:"rgba(26,29,46,.05)",borderWidth:3,borderDash:[6,3],pointRadius:6,tension:.35,spanGaps:true,fill:true,pointBackgroundColor:"#1a1d2e",pointBorderColor:"#fff",pointBorderWidth:2});
      ...
      <div ref={resRef} className="app-shell">
        ...
        <div className="main-panel">
          <div className="panel-body">
                ...
                <div className="grade-section">
                  ...
                  <div className="grade-row">
                    <div className="tbl-card">
                      <div style={{padding:"14px 16px 10px",borderBottom:"1px solid var(--border)",fontWeight:700,fontSize:13}}>📋 학기별 성적표</div>
                    ...
                    <div className="chart-card">
                      <div className="chart-lbl">학기별 등급 추이</div>
                      <div className="chart-sub">교과군별 색상 · 전체평균 점선</div>
            ...
            {sec==="s2"&&(
              <div>
                <PH eye="이수 분析" title="교과 이수 현황" sub="충족률 높은 순"/>
                {G?.creditAnalysis?.length?(
            ...
            {sec==="s5"&&(
              <div>
                <PH eye="언어 분析" title="키워드 분析"/>
                {G?.keywords?.length?(
                  <>
                    <div className="card">
                      <div className="card-eye">핵심 키워드 클라우드</div>
                      <div className="kw-cloud">
                        {G.keywords.slice(0,40).map((k,i)=>{
                          const sz=Math.max(12,Math.min(20,12+Math.floor((40-i)/40*8)));
                          return <span key={i} className="kw-tag" style={{fontSize:sz,color:TC[i%TC.length],background:TC[i%TC.length]+"16",border:`1px solid ${TC[i%TC.length]}28`}}>{k}</span>;
                        })}
                      </div>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginTop:14}}>
                      ...
                      <div className="card">
                        ...
                        {G.methodBias?.dominant&&<div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
                          ...
                          <span style={{fontSize:12,fontWeight:700,padding:"5px 12px",borderRadius:3,background:G.methodBias.isSkewed?"var(--red-l)":"var(--green-l)",color:G.methodBias.isSkewed?"var(--red)":"var(--green)",border:`1px solid ${G.methodBias.isSkewed?"#fca5a5":"#b2f2bb"}`}}>{G.methodBias.isSkewed?"⚠ 편중":"✓ 균형"}</span>
                        </div>}
            ...
            {sec==="s9"&&(
              <div>
                <PH eye="학과 분析" title="학과 적합도" sub="생기부 원문 근거 기반"/>
                {G?.majorFit?.length?G.majorFit.slice(0,5).map((m,i)=>(
            ...
            {sec==="s10"&&(
              <div>
                <PH eye="탐구 제안" title="심화탐구 주제 제안" sub="활동 기반 · 구체적 방법론"/>
                {G?.topics?.length?(
                  <div className="topic-grid">
                    {G.topics.slice(0,6).map((t,i)=>(
                      <div key={i} className="topic-card">
                        <div className="topic-stripe" style={{background:TC[i%TC.length]}}/>
                        <div className="topic-num">탐구 주제 {i+1}</div>
                        <div className="topic-title">{t.title}</div>
                        {t.basedOn&&<div className="topic-based">{t.basedOn}</div>}
                        <div className="topic-reason">{t.reason}</div>
                        <div className="topic-meta">
                          {t.method&&<span className="tm-method">{t.method}</span>}
                          {t.difficulty&&<span className="tm-diff">난이도: {t.difficulty}</span>}
                          {t.benefit&&<span className="tm-benefit">✓ {t.benefit}</span>}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {sec==="s11"&&(
              <div>
                <PH eye="면접 대비" title="예상 면접 질문" sub="생기부 기반 개인화"/>
                {G?.interviewQs?.length?(
                  <div className="iq-list">
                    {G.interviewQs.slice(0,5).map((q,i)=>(
                      <div key={i} className="iq-card">
                        <div className="iq-q">
                          <span className="iq-num" style={{color:TC[i%TC.length]}}>Q{i+1}</span>
                          <span className="iq-text">{q.question}</span>
                        </div>
                        {q.basedOn&&<div className="iq-based">📍 근거: {q.basedOn}</div>}
                        {q.tip&&<div className="iq-tip"><strong style={{color:"var(--blue)"}}>답변 팁: </strong>{q.tip}</div>}
                        {q.pitfall&&<div className="iq-pitfall">⚠ 주의: {q.pitfall}</div>}
                      </div>
                    ))}
                  </div>
                ):<Empty/>}
            ...
            {sec==="s12"&&(
              <div>
                <PH eye="종합 리포트" title="정밀 분析 리포트" sub={`${G?.schoolName||""} · ${name} 학생`}/>
                ...
                <div className="letter-wrap">
                  ...
                  <div className="letter-body">{G?.reportLetter||"분析 실행 후 생성됩니다."}</div>
                ...
                {G?.competencies&&(
                  <div className="card mt20">
                    <div className="card-eye">역량별 채점 요약</div>
                ...
                <div style={{marginTop:14,padding:"12px 16px",background:"#f0f2f8",borderRadius:6,border:"1px solid var(--border)",fontSize:12,color:"var(--ink3)",lineHeight:1.8}}>
                  리포트아이 (Report-I) · 2026 합격생 빅데이터 기준 · {new Date().toLocaleDateString("ko-KR")}<br/>
                  본 리포트는 학생부 원문 분析 기반이며 실제 입시 결과와 차이가 있을 수 있습니다.
                </div>
              </div>
            )}
          </div>
